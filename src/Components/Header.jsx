...
const Header = ({ isDarkMode, toggleTheme }) => {
  ...
  useEffect(() => {
    Object.values(dropdownRefs.current).forEach((el) => {
      if (!el) return;
      gsap.set(el, { opacity: 0, y: -10, display: 'none' }); // hidden initially
    });
  }, []);

  useEffect(() => {
    Object.entries(dropdownRefs.current).forEach(([label, el]) => {
      if (!el) return;
      if (openDropdown === label) {
        ...
        gsap.to(el, { opacity: 0, y: -10, display: 'none', duration: 0.2, ease: 'power2.in' });
      }
    });
  }, [openDropdown]);
    ...
    <div className="cursor-default p-2 md:p-0 px-2 absolute top-0 w-full">
      <div className="px-3 lg:container mx-auto flex justify-between items-center h-full">
        ...
        <div className="flex-1 hidden lg:block">
          <ul style={{ fontFamily: 'PovetaracSansBold' }} className="flex text-[#111111] justify-center items-center gap-6 relative">
            {menuItems.map(({ label, dropdown }) => (
              <li key={label} className={`text-[18px] py-4 cursor-pointer flex items-center gap-2 relative ${  isDarkMode ? 'text-white' : 'text-black'}`}
                onMouseEnter={() => setOpenDropdown(label)}
                onMouseLeave={() => setOpenDropdown(null)}
              >
                <span>{label}</span>
                <FaChevronDown className={`w-3 h-3 stroke-[1.5] transition-transform duration-300 ${  openDropdown === label ? 'rotate-180' : 'rotate-0' } ${isDarkMode ? 'text-[#ffffff]' : 'text-[#000000]'}`} />

                {/* Dropdown */}
                <ul ref={(el) => (dropdownRefs.current[label] = el)} className="dropdown-hidden absolute top-full left-0 text-lg z-50 bg-white dark:bg-[#1a1a1a] overflow-hidden rounded-lg shadow-lg w-80 border border-[#ebf0f5] text-black dark:text-white font-normal" style={{ fontFamily: 'PovetaracSansBold' }} >
        ...
        <div className="w-full max-w-[320px] items-center justify-end hidden lg:flex">
          <div className="flex items-center gap-6">
            <Link to="/login" style={{ fontFamily: 'PovetaracSansBold' }} className={`text-lg underline underline-offset-8 ${isDarkMode ? 'text-white' : 'text-black'}`}>
              Login
            </Link>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 bg-white dark:bg-[#1a1a1a] p-2 z-[100] flex flex-col">
          ...
          <div className="px-3 w-full mx-auto flex justify-between items-center">
             ...
             <div className="lg:hidden flex items-center">
                <div className='bg-gray-100 p-2 rounded-2xl'>
                   <svg xmlns="http://www.w3.org/2000/svg"  className="cursor-pointer" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={isDarkMode ? "white" : "black"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" onClick={() => setMobileOpen(false)}>
          ...
          <div className="flex flex-col gap-2 overflow-scroll pt-6 p-2">
             <div className={`flex justify-between items-center px-4 py-4 rounded-2xl bg-gray-50 cursor-pointer`} onClick={() => {navigate('/');setMobileOpen(false)}}>
                  <span className="text-lg font-bold">Home</span>
             </div>
            {menuItems.map(({ label, dropdown }) => (
              <div key={label} className="">
                <div className={`flex justify-between items-center px-4 py-4 bg-gray-50 cursor-pointer ${  expandedSection === label ? 'rounded-t-xl ' : 'rounded-xl'  }`} onClick={() => toggleSection(label)}>
                  <span className="text-lg font-bold">{label}</span>
                  <FaChevronDown className={`transition-transform duration-300 ${  expandedSection === label ? 'rotate-180' : ''  }`} />
                </div>
                ...
                {expandedSection === label && (
                  <ul className="pb-6 bg-gray-50 px-5 rounded-b-2xl space-y-2">
                    {dropdown.map(({ title }) => (
                      <li key={title} className="text-[16px] cursor-pointer hover:text-[#00C2CC]" onClick={() => setMobileOpen(false)} >
                        {title}
                      </li>
                    ))}
                  </ul>
